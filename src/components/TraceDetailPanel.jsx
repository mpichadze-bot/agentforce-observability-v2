...
function getOrchestrationByMessage(trace) {
  ...
  const findOrchestration = (spans) => {
    spans.forEach(span => {
      ...
      const isSubAgent = span.name === 'agent.handoff' || 
      ...
      if (isSubAgent) {
        ...
        // Clean up agent name
        agentName = agentName.replace('A2A: ', '').replace('agent.handoff', 'Sub-Agent');
      ...
      if (isMCP) {
        let toolName = span.attributes?.['mcp.tool.name'] || 
      ...
      // Check nested spans
      if (span.children) {
        findOrchestration(span.children);
      }
    });
  };
...
function SessionLogPanel({ sessionLog, trace, sessionDate, onMessageClick }) {
  ...
  const renderOrchestration = (messageIndex) => {
    ...
    const totalCount = orchestration.subAgents.length + orchestration.mcps.length;
    const isExpanded = expandedOrchestration[messageIndex] ?? false;
    
    // If only 1 item, show it directly without collapse
    if (totalCount <= 1) {
      return (
      ...
      <div className="flex flex-col gap-1.5">
        ...
        >
          ...
          <span className="font-medium">
            {orchestration.subAgents.length > 0 && orchestration.mcps.length > 0
              ? `${orchestration.subAgents.length} agent${orchestration.subAgents.length !== 1 ? 's' : ''}, ${orchestration.mcps.length} MCP${orchestration.mcps.length !== 1 ? 's' : ''}`
...
// Helper: Calculate routing overhead and response time for sub-agent calls
function getSubAgentLatencyBreakdown(item, traceItems) {
  // Check if this is a sub-agent (A2A handoff)
  ...
  const findActionSelection = (items, targetTime) => {
    ...
    const traverse = (itemList) => {
      itemList.forEach(it => {
        ...
        const itEndTime = itStartTime + (it.duration || 0);
        
        if (isActionSelection && itEndTime <= targetTime) {
          if (!lastActionSelection || itEndTime > lastActionSelection.endTime) {
            lastActionSelection = {
...
function TraceItem({ item, index, isExpanded, isSelected, onToggle, onSelect, depth, expandedItems, toggleExpand, setSelectedAction, selectedAction, traceItems }) {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Latency Breakdown for Sub-Agents */}
        {(() => {
